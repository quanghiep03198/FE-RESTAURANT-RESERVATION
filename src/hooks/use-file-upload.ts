		...
		(file: File | FileMetadata): string | null => {
			if (file instanceof File) {
				if (file.size > maxSize) {
					return `Tệp "${file.name}" vượt quá dung lượng tối đa ${formatBytes(maxSize)}.`
				}
			} else {
				if (file.size > maxSize) {
					return `Tệp "${file.name}" vượt quá dung lượng tối đa ${formatBytes(maxSize)}.`
			...
			if (accept !== '*') {
				...
				if (!isAccepted) {
					return `Tệp "${file instanceof File ? file.name : file.name}" không đúng định dạng được chấp nhận.`
				}
			}
		...
		(newFiles: FileList | File[]) => {
			...
			for (const file of newFilesArray) {
				...
				if (file.size > maxSize) {
					errors.push(
						multiple
							? `Một số tệp vượt quá dung lượng tối đa ${formatBytes(maxSize)}.`
							: `Tệp vượt quá dung lượng tối đa ${formatBytes(maxSize)}.`
					)
					continue
				}
