...
const TableHead: React.FC<React.ComponentProps<'th'>> = ({ className, ...props }) => (
	<th
		className={cn(
			'bg-background text-table-head-foreground group-hover:bg-secondary/50 w-full px-4 py-2 font-semibold data-[type=number]:text-right [&:has([role=button])]:text-center [&:has([role=checkbox])]:text-center [&:has([role=combobox])]:p-0 [&:has([role=listbox])]:p-0 [&:has([role=textbox])]:p-0',
			className
		)}
		{...props}
	/>
...
const TableCell: React.FC<React.ComponentProps<'td'>> = ({ className, ...props }) => (
	<td
		className={cn(
			'bg-background group-hover:bg-table-row-active group-aria-expanded:bg-table-row-active group-aria-selected:bg-table-row-selected data-[disabled=true]:bg-muted px-4 py-2 first:border-l-0 last:border-r-0 data-[type=number]:text-right [&:has([role=button])]:text-center [&:has([role=checkbox])]:text-center [&:has([role=combobox])]:p-0 [&:has([role=listbox])]:p-0 [&:has([role=textbox])]:p-0',
			className
		)}
		{...props}
	/>
